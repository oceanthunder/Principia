...
// #error This program is not (yet) prepared for big endian CPUs, please contact
// the author.
#endif
...
// It used to be #undefined at the end of that file, but since some time in 2006
// it's kept available. And SDL's definition changed in SDL 2.0.6, which caused
// a warning at this redefinition. So we should just use the macro from SDL and
// not define our own.
...
typedef int (*add_table_type)(short chtab_id, int id, sbyte xh, sbyte xl,
                              int ybottom, int blit, byte peel);
...
enum blitters {
  ...
  // It seems to me that the "or" blitter can be safely replaced with the
  // "transparent" blitter.
  blitters_2_or = 2,
...
enum grmodes { gmCga = 1, gmHgaHerc = 2, gmEga = 3, gmTga = 4, gmMcgaVga = 5 };
...
// On the original levels, roomxs[] and roomys[] contain the x/y coordinates of
// the rooms. They were used by Mechner's level editor.

// fill_2[] is used by RoomShaker: If you place a Debug Start in RoomShaker,
// the start_* fields will contain the location of the Debug Start, and fill_2[]
// will contain the location of the Player Start.
...
typedef struct char_type {
  ...
  byte action;
  sbyte fall_x;
  sbyte fall_y; // Falling speed, but also used to check falling distance (less
                // than 22 = one row, less than 33 = two rows)
  byte room;
  byte repeat;
  byte charid;
  byte sword;
...
enum sword_status { sword_0_sheathed = 0, sword_2_drawn = 2 };
...
enum directions { dir_0_right = 0x00, dir_56_none = 0x56, dir_FF_left = -1 };
...
enum replay_special_moves {
  ...
  MOVE_EFFECT_END = 2,    // music stops, causing the end of feather effect or
                          // level 1 crouch immobilization
};
...
// Macros for declaring and initializing a names_list_type (for names lists and
// key/value pair lists).
#define NAMES_LIST(listname, ...)                                              \
  const char listname[][MAX_OPTION_VALUE_NAME_LENGTH] = __VA_ARGS__;           \
  names_list_type listname##_list = {.type = 0,                                \
                                     .names = {&listname, COUNT(listname)}}

#define KEY_VALUE_LIST(listname, ...)                                          \
  const key_value_type listname[] = __VA_ARGS__;                               \
  names_list_type listname##_list = {                                          \
      .type = 1, .kv_pairs = {(key_value_type *)&listname, COUNT(listname)}}
...
enum {
  ...
  KEYSTATE_HELD_NEW =
      (1 << 1), // Key has been pressed since last gameplay update
};

// Enum used for input variables like control_up, control_forward,
// control_shift2, and etc
enum {
  // for all control_* variables:
  CONTROL_RELEASED = 0,
  // for control_shift, control_forward, control_backward, control_up,
  // control_down, control_shift2:
  CONTROL_IGNORE = 1,
  CONTROL_HELD = -1,
...
#define TILE_SIZEX                                                             \
  14 // Horizontal size of tile in the internal coordinate system (a tile is 32
     // pixels wide in screen space)
#define TILE_MIDX 7    // Middle horizontal point of a tile
#define TILE_RIGHTX 13 // Right-most point of a tile
#define TILE_SIZEY                                                             \
  63 // Vertical size of a tile (this also matches the pixel height of tiles in
     // screen space)
#define SCREENSPACE_X                                                          \
  58 // Position of the left-most pixel in screenspace in the internal
     // coordinate system
...
#define FIRST_ONSCREEN_COLUMN                                                  \
  5 // Used for referencing the first column visible in screen space in the
    // x_bump array
...
#define SCREEN_GAMEPLAY_HEIGHT                                                 \
  192 // Portion of the screen space dedicated to gameplay graphics
