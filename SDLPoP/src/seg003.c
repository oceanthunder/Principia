...
// Save a frame to a file for RL debugging
...
int play_level_2() {
  ...
  while (1) { // main loop
    ...
    if (rl_step_mode) {
      ...
      update_screen();
      idle();
      // Optional: limit frame rate for visual debugging (comment out for max
      // speed) do_simple_wait(timer_1);  // DISABLED for max RL speed
